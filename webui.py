...
def load_model(cache_model):
    ...
    bert_model.load_state_dict(torch.load(f'model/{cache_model}.pt',map_location={'cuda:2': 'cuda:0'}))
...
def generate_prompt(response,query,client,bert_model, bert_tokenizer,rule, tfidf_r, device, idx2tag):
    ...
    # print(entities)
    yitu = []
    prompt = "<指令>你是一个医疗ollama run qwen:32b问答机器人，你需要根据给定的提示回答用户的问题。请注意，你的全部回答必须完全基于给定的提示，不可自由发挥。如果根据提示无法给出答案，立刻回答“根据已知信息无法回答该问题”。</指令>"
...
def main(is_admin, usname):
    ...
    client = py2neo.Graph('bolt://localhost:7687', user='neo4j', password='zhy2003246', name='neo4j')
